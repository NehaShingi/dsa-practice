...
The upper bound is the smallest index, ind, where arr[ind] > x. But if any such index is not found, the lower bound algorithm returns n i.e. size of the given array.
...
int findUpperBound(vector<int> &arr, int key)
{
    ...
    while (low <= high)
    {
        ...
        {
            // arr[mid]<=key, so search in right half
            low = mid + 1;
        }
    }
    return ans;
